...
export class ItemPackageDatabaseProvider implements DatabaseProvider<LayeredItemPackageRepository> {
  public readonly id: DatabaseProviderId = 'package';
  public readonly name = 'Item Packages';

  private repository?: LayeredItemPackageRepository;
  ...
  public getMetadata(): DatabaseMetadata {
    return {
      id: this.id,
      name: this.name,
      entityCount: this.repository ? this.repository.getAllEffectivePackages().length : 0,
  ...
  public async load(context: DatabaseContext): Promise<void> {
    if (!context.workspacePath) {
      throw new Error('Workspace path is required to load Item Packages Database.');
    }
    ...
    try {
      ...
      const { repository, parseResults } = await this.loader.loadItemPackageRepositoryFromProvider(
        context.variant,
        provider,
        context.layers
      );
  ...
  public async reloadLayer(layerId: string): Promise<void> {
    ...
    const partialResult = await this.loader.loadItemPackageRepositoryFromProvider(this.lastContext.variant, fileProvider, [layerDef]);
  ...
  public getRepository(): LayeredItemPackageRepository | undefined {
