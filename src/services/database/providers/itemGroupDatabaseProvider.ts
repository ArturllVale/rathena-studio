...
export class ItemGroupDatabaseProvider implements DatabaseProvider<LayeredItemGroupRepository> {
  public readonly id: DatabaseProviderId = 'group';
  public readonly name = 'Item Groups';

  private repository?: LayeredItemGroupRepository;
  ...
  public getMetadata(): DatabaseMetadata {
    return {
      id: this.id,
      name: this.name,
      entityCount: this.repository ? this.repository.getAllEffectiveGroups().length : 0,
  ...
  public async load(context: DatabaseContext): Promise<void> {
    if (!context.workspacePath) {
      throw new Error('Workspace path is required to load Item Group Database.');
    }
    ...
    try {
      ...
      const { repository, parseResults } = await this.loader.loadItemGroupRepositoryFromProvider(
        context.variant,
        provider,
        context.layers
      );
  ...
  public async reloadLayer(layerId: string): Promise<void> {
    ...
    const partialResult = await this.loader.loadItemGroupRepositoryFromProvider(this.lastContext.variant, fileProvider, [layerDef]);
  ...
  public getRepository(): LayeredItemGroupRepository | undefined {
